...
Create a `integration-base-image.json` file inside the `packer` directory with the following content (make sure to change the project ID and zone in case it's different):
  ...
  "builders": [
    {
      ...
      "image_name": "integration-base-{{isotime `20060102-150405`}}",
      "image_family": "integration-base",
      "image_description": "Ubuntu 16.04 with Ruby, Bundler and MongoDB preinstalled",
      "ssh_username": "integration-user"
    }
  ]
}
```
...
```bash
$ packer validate ./packer/integration-base-image.json
```
...
```bash
$ packer build ./packer/integration-base-image.json
```
...
```bash
$ INSTANCE_IP=$(gcloud --format="value(networkInterfaces[0].accessConfigs[0].natIP)" compute instances describe integration-instance-4)
...
```bash
$ gcloud --format="value(networkInterfaces[0].accessConfigs[0].natIP)" compute instances describe integration-instance-4
```
...
```bash
$ gcloud compute instances delete integration-instance-4
```
