...
```bash
$ gcloud compute instances create integration-instance-3 \
...
```bash
$ git clone https://github.com/GaabsDev/iac-tutorial.git
```
...
mv integration.service /etc/systemd/system/integration.service
```
...
```bash
$ INSTANCE_IP=$(gcloud --format="value(networkInterfaces[0].accessConfigs[0].natIP)" compute instances describe integration-instance-3)
$ scp -r ./scripts integration-user@${INSTANCE_IP}:/home/integration-user
```
...
```bash
$ gcloud --format="value(networkInterfaces[0].accessConfigs[0].natIP)" compute instances describe integration-instance-3
```
...
```bash
$ gcloud compute instances delete integration-instance-3
```
