...
```bash
$ gcloud compute instances create integration-instance-4 \
    --image-family integration-base \
    --boot-disk-size 10GB \
...
```
resource "google_compute_instance" "integration" {
  ...
  boot_disk {
    initialize_params {
      image = "integration-base" // use image built with Packer
    }
  }
...
```bash
$ gcloud compute instances describe integration-instance
```
...
```bash
$ INSTANCE_IP=$(gcloud --format="value(networkInterfaces[0].accessConfigs[0].natIP)" compute instances describe integration-instance)
$ scp ../scripts/deploy.sh integration-user@${INSTANCE_IP}:/home/integration-user
```
...
```bash
$ gcloud --format="value(networkInterfaces[0].accessConfigs[0].natIP)" compute instances describe integration-instance
```
...
$ gcloud compute firewall-rules delete allow-integration-tcp-9292
```

Make sure that your application became inaccessible via port 9292 and SSH connection with a private key of `integration-user` fails.
...
resource "google_compute_project_metadata" "integration" {
  metadata {
    ssh-keys = "integration-user:${file("~/.ssh/integration-user.pub")}" // path to ssh key file
  }
}

resource "google_compute_firewall" "integration" {
  name    = "allow-integration-tcp-9292"
  network = "default"
  allow {
    protocol = "tcp"
    ports    = ["9292"]
...
```
output "integration_public_ip" {
  value = "${google_compute_instance.integration.network_interface.0.access_config.0.assigned_nat_ip}"
}
```
