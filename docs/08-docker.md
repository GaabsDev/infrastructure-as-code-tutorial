...
Let's use these instructions to create a Docker container image for our integration application.
...
RUN mkdir $APP_HOME
WORKDIR $APP_HOME
COPY integration-app/Gemfile* $APP_HOME/
RUN bundle install

# Copy the application code to the container
ADD integration-app/ $APP_HOME
...
Once you defined how your image should be built, run the following command inside `iac-tutorial` directory to create a container image for integration application:

```bash
$ docker build --tag integration .
```

The resulting image will be named `integration`. Find it in the list of your local images:
...
We will use a public image from Docker Hub to run a MongoDB container alongside integration application container. However, I recommend you for the sake of practice write a Dockerfile for MongoDB and create your own image.
...
```bash
$ docker run --name integration-app \
